(function(){
  ...
  var FEATURE_LABELS={free:'免費遊戲',awaken:'覺醒之力',immortal:'不朽覺醒'};
  // 倍數是從單一場真實截圖反推的（押注20時：免費遊戲4,000／覺醒之力10,000／不朽覺醒40,000），
  // 不是官方公式。只驗證過這一組押注，若別的押注金額比例不同，改這裡三個數字就好。
  var FEATURE_MULTIPLIER={free:200,awaken:500,immortal:2000};
  var featureType='free';
  ...
  var ratio=function(part,whole){return whole>0?(part/whole)*100:0};
  ...
  function makeEvent(type,label,balance,delta){return{id:eventId(),type:type,timestamp:Date.now(),label:label,balance:balance,delta:delta}}
  ...
  function renderTimeline(){el('timeline-list').innerHTML=session.events.slice(0,5).map(function(item){var time=new Date(item.timestamp).toLocaleTimeString('zh-TW',{hour:'2-digit',minute:'2-digit'});return'<div class="timeline-row"><time>'+time+'</time><span>'+item.label+'</span><b>$'+money(item.balance)+'</b></div>'}).join('')}
  ...
  function addSpinAmount(add,win){if(!session||add<=0)return;win=Math.max(0,win||0);var cost=add*session.currentBet,delta=win-cost,balance=Math.max(0,session.currentBalance+delta);session.totalSpins=(session.totalSpins||0)+add;session.currentBalance=balance;session.lowestBalance=Math.min(session.lowestBalance,balance);var label='轉了 '+add+' 次，花費 $'+money(cost)+(win>0?'，贏回 $'+money(win):'');session.events.unshift(makeEvent('spins',label,balance,delta));el('spin-draft').value='';el('spin-win-draft').value='';el('balance-draft').value=balance;save();renderActive();announce('已加入 '+add+' 轉，'+(win>0?'淨賺 $'+money(delta):'花費 $'+money(cost))+'，累計 '+session.totalSpins+' 轉');track('spin_count_add',{added:add,had_win:win>0})}
  function addSpins(){if(!session)return;var add=Math.round(num('spin-draft'));if(add<=0){announce('請輸入大於零的轉數');return}addSpinAmount(add,num('spin-win-draft'))}
  function addSpinChip(event){var add=Number(event.currentTarget.dataset.spins)||0;addSpinAmount(add,num('spin-win-draft'))}
  ...
  function endPayload(){
    ...
    return{
      ...
      feature_net:featNet,
      total_spins:session.totalSpins||0,
  ...
  function resetSession(){session=null;save();['starting-bankroll','starting-bet','loss-limit','profit-target'].forEach(function(id,index){el(id).value=[2000,6,1000,500][index]});featureType='free';document.querySelectorAll('.type-chip').forEach(function(chip){chip.classList.toggle('active',chip.dataset.type==='free')});renderSetup();toggleState('setup');announce('已清除上一場記錄')}
  function restore(){try{session=JSON.parse(localStorage.getItem(STORAGE_KEY)||'null')}catch(error){session=null}if(!session){toggleState('setup');renderSetup();track('session_tool_open');return}el('balance-draft').value=session.currentBalance;el('bet-draft').value=session.currentBet;el('compare-bet').value=session.currentBet;applyFeatureCost();if(session.status==='ended'){toggleState('ended');renderSummary()}else{toggleState('active');renderActive();startTimer()}track('session_tool_open',{restored_session:true})}

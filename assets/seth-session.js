(function(){
  ...
  function addSpins(){if(!session)return;var add=Math.round(num('spin-draft'));if(add<=0){announce('請輸入大於零的轉數');return}session.totalSpins=(session.totalSpins||0)+add;session.events.unshift(makeEvent('spins','轉了 '+add+' 次',session.currentBalance));el('spin-draft').value='';save();renderActive();announce('已加入 '+add+' 轉，累計 '+session.totalSpins+' 轉');track('spin_count_add',{added:add})}
  ...
  el('setup-form').addEventListener('submit',startSession);['starting-bankroll','starting-bet','loss-limit','profit-target'].forEach(function(id){el(id).addEventListener('input',renderSetup)});el('update-balance').addEventListener('click',updateBalance);el('update-bet').addEventListener('click',updateBet);el('add-spins').addEventListener('click',addSpins);el('record-feature').addEventListener('click',recordFeature);el('end-session').addEventListener('click',endSession);el('reset-session').addEventListener('click',resetSession);['compare-spins','compare-bet','compare-feature'].forEach(function(id){el(id).addEventListener('input',function(){if(session)renderCompare(metrics())})});el('open-guide').addEventListener('click',openGuide);el('guide-close').addEventListener('click',function(){closeGuide(false)});el('guide-skip').addEventListener('click',function(){closeGuide(false)});el('guide-next').addEventListener('click',nextGuide);el('guide-back').addEventListener('click',previousGuide);document.addEventListener('keydown',guideKeyboard);restore();if(!localStorage.getItem(GUIDE_KEY))setTimeout(openGuide,450);
})();

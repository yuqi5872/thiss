(function(){
  ...
  function addSpinAmount(add){if(!session||add<=0)return;session.totalSpins=(session.totalSpins||0)+add;session.events.unshift(makeEvent('spins','轉了 '+add+' 次',session.currentBalance));el('spin-draft').value='';save();renderActive();announce('已加入 '+add+' 轉，累計 '+session.totalSpins+' 轉');track('spin_count_add',{added:add})}

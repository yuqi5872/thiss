...
(function () {
  ...
  var SEG_LENS = [3, 3, 2];
  /* 各工具需要的等級。0 = 完全免費，不擋。
     2026-08-13 user 定案的階梯（照工具的實際價值排，不是照做的難度）：
       免費        機制模擬器
       註冊        戰局計算、AI 選房數據助手
       儲值 2,000  分注打法＋照打法追蹤，送觀測手冊
       儲值 3,000  彩金水位判讀           ← 最有價值的擋最高
     戰局計算與 AI 選房從 2,000/3,000 降到註冊，是因為它們的價值撐不起
  ...
  function bindSession() {
    ...
    gates.session = panel({
      ...
      title: '戰局計算的免費試用是一場',
      copy: '你已經完整記錄過一場。想繼續記錄下一場，需要在合作平台<b>完成註冊</b>——' +
            '加 LINE 傳「帳務明細－存款全部狀態」截圖（能看到累計存款欄），系統核對後會自動給你解鎖碼。' +
  ...
  function bindAi() {
    ...
    gate.innerHTML =
      ...
      '<h3 style="margin:0 0 10px;font-size:17px;color:' + CREAM + '">AI 選房數據助手・完成註冊開啟</h3>' +
      '<p style="margin:0 auto 16px;max-width:34em;font-size:14.5px;line-height:1.85;color:rgba(242,234,220,.72)">' +
        '這一項需要在合作平台<b>完成註冊</b>之後開啟。' +
        '加 LINE 傳「帳務明細－存款全部狀態」截圖（能看到累計存款欄），系統核對後會自動給你解鎖碼。<br>' +
